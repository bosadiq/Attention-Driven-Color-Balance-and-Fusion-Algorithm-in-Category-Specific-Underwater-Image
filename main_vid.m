% Underwater Video Enhancement Using Attention-Guided Autoencoder Framework
close all;
clear;
clc;

%% Load the video
videoObj = VideoReader('ArmyDiver1.mp4'); 
outputVideo = VideoWriter('enhanced_underwater_video.mp4', 'MPEG-4');
...
while hasFrame(videoObj)
    % Read current frame
    rgbImage = double(readFrame(videoObj)) / 255;
    
    % Validate input
    if size(rgbImage, 3) ~= 3
        error('Frame must be RGB.');
    end
...
toc;
    ...
    writeVideo(outputVideo, im2uint8(final_output_rgb));
end

%% Close video writer
close(outputVideo);

%% Aggregate Evaluation Metrics (Optional - Process a reference video or frames)
% Note: For video metrics, you may need a reference video or frame sequence
% This example uses the last frame for demonstration
referenceFrame = double(imread('Reference_1.png')) / 255; % Replace with reference frame or video
FSIM_value = fsim_color(im2uint8(final_output_rgb), im2uint8(referenceFrame));
PSNR_value = psnr(im2uint8(final_output_rgb), im2uint8(referenceFrame));
